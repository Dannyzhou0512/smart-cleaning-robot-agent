from Agent_project.utils.config_handler import prompts_config
...
def load_query_rewrite_prompts():
    try:
        query_rewrite_prompt_path = get_abs_path(prompts_config['query_rewrite_prompt_path'])
    except KeyError as e:
        logger.error(f"[load_query_rewrite_prompts]在yaml配置项中没有query_rewrite_prompt_path配置项")
        raise e

    try:
        return open(query_rewrite_prompt_path, "r", encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_query_rewrite_prompts]解析查询改写提示词出错, {str(e)}")
        raise e
